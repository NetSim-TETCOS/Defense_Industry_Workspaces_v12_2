...
int fn_NetSim_TDMA_TransmitPacket(NetSim_PACKET* packet)
{
	...
	if (DEVICE_MACLAYER(packet->nReceiverId, 1)->nMacProtocolId != MAC_PROTOCOL_TDMA)
		return 0;
	if (DEVICE_MACLAYER(packet->nTransmitterId, 1)->nMacProtocolId != MAC_PROTOCOL_TDMA)
		return 0;
...
int fn_NetSim_TDMA_CalulateReceivedPower()
{
	...
	{
		if (DEVICE_MACLAYER(i + 1, 1)->nMacProtocolId != MAC_PROTOCOL_TDMA)
			continue;
		for (j = 0; j < NETWORK->nDeviceCount; j++) {
			if (DEVICE_MACLAYER(j + 1, 1)->nMacProtocolId != MAC_PROTOCOL_TDMA)
				continue;
			if (i != j)
